...
1. 客户端发送ssl链接请求给服务端
2. 服务端发送公钥、证书向客户端
3. 客户端收到后，检查证书的颁发机构以及过期时间，如果没问题生成一个私钥，通过公钥加密发送给服务端
4. 服务端用自己的私钥解密客户端发来的会话私钥
5. 后续用会话私钥进行通信
...
2. 检查操作系统缓存
3. 检查路由器缓存
4. 向根服务器请求 -> .com 请求 -> .baidu 请求 依次向前递归查询

### web安全防范

**XSS**：跨站脚本攻击，就是攻击者想尽一切办法将可以执行的代码注入到网页中
1. 将脚本代码通过输入框注入到数据库中：代码从数据库取出来之后，作为 html 渲染，恶意代码会直接执行
2. 修改浏览器URL：如：https://www.aa.com?kw=<script>alert("xss")</script>，诱导用户点击，浏览器使用 X-XSS-Protection 自动防御，但不是所有浏览器都支持 X-XSS-Protection 
防御：不信任任何客户端提交的数据，只要是客户端提交的数据就应该先进行相应的过滤处理然后方可进行下一步的操作（转码等），cookie 设置为 HttpOnly（通过js脚本将无法读取到cookie信息）
 
**CSRF**：跨站请求伪造，伪造用户的浏览器的请求，向访问一个用户自己曾经认证访问过的网站发送出去，使目标网站接收并误以为是用户的真实操作而去执行命令，比如将 img 标签中 src 赋值为一个请求地址
防范：
1. Get 请求不用于对数据进行修改
2. 接口设置禁止跨域
3. 请求时附带验证信息，比如验证码或者 Token

**点击劫持**：点击劫持是一种视觉欺骗的攻击手段,通过覆盖不可见的框架误导受害者点击。虽然受害者点击的是他所看到的网页，但其实他所点击的是被黑客精心构建的另一个置于原网页上面的透明页面。攻击者将需要攻击的网站通过 iframe 嵌入自己的网页中，并将 iframe 设置为透明，然后诱使用户在该页面上进行操作，此时用户将在不知情的情况下点击透明的iframe页面

防范：X-Frame-Options，有三个值  
DENY：浏览器会拒绝当前页面加载任何frame页面；  
SAMEORIGIN：frame页面的地址只能为同源域名下的页面；  
ALLOW-FROM origin：允许frame加载的页面地址  

**中间人攻击**：中间人截取首次传输的密钥并传递给对方自己伪造的信息，证书机制解决了客户端和服务端的信任问题，从而较为有效的防止了中间人攻击
